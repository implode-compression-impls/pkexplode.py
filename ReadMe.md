...
Underlying `libexplode` has critical memory-safety bugs (that [were wontfixed](https://github.com/ladislav-zezula/StormLib/issues/230))! DO NOT USE THIS LIB! USE [`pkblast.py`](https://github.com/implode-compression-impls/pkblast.py) or [``] for **decompression** instead! For **compression** it seems to be OK to use.

This impl crashes on the test sets: `implode-decoder` and `unshield.rs`.

Installation
------------

In order to make it work you need a package with `liblast` itself installed into your system using your distro package manager. If your distro doesn't provide one, you can build it yourself using CMake CPack from the sources [by the link](https://github.com/implode-compression-impls/libexplode). You will get 2 packages, one with the headers and another one with the shared library. Only the one with the lib is mandatory.
